...
class TRI():
    ...
    optional_configs_names = ["load_saved_pretreatment", "add_non_saved_anonymizations",
        ...
        "finetuning_learning_rate", "finetuning_sliding_window", "dev_set_column_name","experiment_tag"]


    experiment_tag = "100.0_independent"
    ...
    def read_data(self) -> pd.DataFrame:
        ...
        # 在read_data方法中，return data_df之前添加：
        if all(col in data_df.columns for col in ['original_abstract', 'masked_abstract', 'noise_abstract']):
            print("前两行数据：")
# 在 read_data 方法中，替换错误的 print 语句
            print(data_df[[self.individual_name_column, 'original_abstract', 'masked_abstract', 'noise_abstract']].head(2))
        return data_df
    ...
    def split_data(self, data_df:pd.DataFrame):
        ...
        eval_columns = ['original_abstract', 'masked_abstract', 'noise_abstract']
        ...
        for col in eval_columns:
            if col in data_df.columns:
                ...
                self.eval_dfs[col] = eval_df
            else:
                print(f"警告: 列 '{col}' 不存在于数据中，跳过。")
        
        return train_df, self.eval_dfs
    #endregion
    ...
    def get_trainer(self, model, task_config, train_dataset, eval_datasets_dict=None, data_collator=None):
        ...
        trainer = TRITrainer(results_filepath,
                            model=model,
                            args=args,
                            save_class_embeddings=save_class_embeddings,  # 👈 传入参数
...
class TRITrainer(Trainer):
    def __init__(self, results_filepath:str = None,save_class_embeddings:bool = True, **kwargs):
        Trainer.__init__(self, **kwargs)
        self.results_filepath = results_filepath
        self.save_class_embeddings = save_class_embeddings  # 👈 添加这行
    ...
    def extract_and_save_class_embeddings(self):
        ...
        try:
            ...
            # 保存距离矩阵
            # output_path = "noise_2.0_independent_distance_matrix.json"
            output_path = f"noise_{EXPERIMENT_TAG}_distance_matrix.json"            
            # 获取 label_to_name 映射（从 TRI 对象）
            # 由于 Trainer 无法直接访问 TRI 对象，这里保存时不包含名称映射
            # 可在后续脚本中加载时补充
            data = {
    ...
    # 在导入部分后添加：
    current_dataset_name = None  # Global variable for dataset name
...
def compute_metrics(results):
    ...
    out = {
        ...
        "probs": probs.tolist(),        # 保留也行
        "logits": np.asarray(logits).tolist(),  # 新增：窗口级 logits
        "labels": labels_np.tolist(),
        "pred_idxs": probs.argmax(axis=1).tolist()
    }

    import json
    filename = f"budget_{EXPERIMENT_TAG}_{current_dataset_name}.json"
    # filename = f"budget_2.0_independent_{current_dataset_name}.json"
